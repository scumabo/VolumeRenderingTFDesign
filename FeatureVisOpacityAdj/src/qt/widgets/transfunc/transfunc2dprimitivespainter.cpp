...
void TransFunc2DPrimitivesPainter::ToggleTransparency()
{
	if (selectedPrimitive_) {
		if(selectedPrimitive_->getColor().a != 255)
		{
			tgt::col4 color = selectedPrimitive_->getColor();
			color.a = 255;
			selectedPrimitive_->setColor(color);
...
void TransFunc2DPrimitivesPainter::paint() {
    ...
    if (showHistogram_ && histogram_ && histogramTex_) {
		...
		/* Bo: load clusters to the canvas */
		...
		// load  the image
		success = ilLoadImage((ILstring)imageFilename.c_str());
		...
		switch(ilGetInteger(IL_IMAGE_FORMAT)) {
			case IL_COLOR_INDEX      : ss =  "IL_COLOR_INDEX"; break;     
			case IL_ALPHA            : ss =  "IL_ALPHA"; break;  
			case IL_RGB              : ss =  "IL_RGB"; break;     
			case IL_RGBA             : ss =  "IL_RGBA"; break;     
			case IL_BGR              : ss =  "IL_BGR"; break;     
			case IL_BGRA             : ss =  "IL_BGRA"; break;     
			case IL_LUMINANCE        : ss =  "IL_LUMINANCE"; break;     
			case  IL_LUMINANCE_ALPHA : ss =  "IL_LUMINANCE_ALPHA"; break;
		}
		printf(" Format %s", ss.c_str());
 
		switch(ilGetInteger(IL_IMAGE_TYPE)) {
			case IL_BYTE           : ss =  "IL_BYTE"; break;     
			case IL_UNSIGNED_BYTE  : ss =  "IL_UNSIGNED_BYTE"; break;    
			case IL_SHORT          : ss =  "IL_SHORT"; break;     
			case IL_UNSIGNED_SHORT : ss =  "IL_UNSIGNED_SHORT"; break;     
			case IL_INT            : ss =  "IL_INT"; break;     
			case IL_UNSIGNED_INT   : ss =  "IL_UNSIGNED_INT"; break;     
			case IL_FLOAT          : ss =  "IL_FLOAT"; break;     
			case IL_DOUBLE         : ss =  "IL_DOUBLE"; break;
			case IL_HALF           : ss =  "IL_HALF"; break;
		}
		printf(" Data type:  %s", ss.c_str());
		...
		float x = IntensityCut/IntensityMax;
		float y = GradientCut/GradientMax;
		//Initialize clear color
		glBegin(GL_QUADS);
			// Front Face
			glColor3f(1.f, 1.f, 1.f);
			glTexCoord2f(0.f, 0.f); glVertex3f(0.f, 0.f, -0.5f);  // Bottom Left Of The Texture and Quad
			glTexCoord2f(1.f, 0.f); glVertex3f(x, 0.f, -0.5f);  // Bottom Right Of The Texture and Quad
			glTexCoord2f(1.f, 1.f); glVertex3f(x, y, -0.5f);  // Top Right Of The Texture and Quad
			glTexCoord2f(0.f, 1.f); glVertex3f(0.f, y, -0.5f);  // Top Left Of The Texture and Quad
		glEnd(); 
		glDisable(GL_TEXTURE_2D);
