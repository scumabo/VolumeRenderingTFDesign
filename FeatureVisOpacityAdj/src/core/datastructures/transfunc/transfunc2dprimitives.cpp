...
// Bo: Default Texture::LINEAR -> Texture::NEAREST
TransFunc2DPrimitives::TransFunc2DPrimitives(int width, int height)
	: TransFunc(width, height, 1, GL_RGBA, GL_FLOAT, Texture::NEAREST)
    , fbo_(0)
...
void TransFunc2DPrimitives::setToStandardFunc() {

	for(int i = 0; i < NumIso; i++)
	{
		/* define the initial opacity of all isosurfaces */
		iniOpacity[i] = 5;
		
		TransFuncPrimitive* p = new TransFuncQuad(map_to_coordinate(isoGradientvalue[i]), 0.1f, tgt::col4(colors[i], iniOpacity[i]));
		addPrimitive(p);
	}
...
void TransFunc2DPrimitives::initiateOpacity() {

	for(int i = 0; i < primitives_.size(); i++)
	{
		iniOpacity[i] = 5;

		// initialize all primitives opacity to defined arbitrary low.
		getPrimitive(i)->setColor(tgt::col4(getPrimitive(i)->getColor().r, getPrimitive(i)->getColor().g, getPrimitive(i)->getColor().b, iniOpacity[i]));
	}
	updateTexture();
}

vec2 TransFunc2DPrimitives::map_to_coordinate(vec2 worldCoordinate){
	/*
	vec2 gradient_range = getDomain(1);
	vec2 intensity_range= getDomain(2);

	vec2 coordinate_tf = vec2(worldCoordinate[0]/IntensityRange*(intensity_range.elem[1]-intensity_range.elem[0]), worldCoordinate[1]/GradientRange*(gradient_range.elem[1] - gradient_range.elem[0]));
	*/
	vec2 coordinate_tf = vec2(worldCoordinate[0]/IntensityMax, worldCoordinate[1]/GradientMax);
	return coordinate_tf;
}
