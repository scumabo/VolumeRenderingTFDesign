...
void TransFunc1DKeys::setToStandardFunc() {
	...
	for(int i = 0; i < NumIso; i++)
	{
		...
		// triangle key
		genKey(getIso(examplar[i]), tgt::col4(colors[i], iniOpacity[i]));

		// square key
		//genSquareKey(getIso(examplar[i]), tgt::col4(colors[i], iniOpacity[i]));
	}
...
/* Create initial transfer functions: delta function at each representative features */
void TransFunc1DKeys::initiateOpacity() {
	for(int i = 0; i < NumIso; i++)
	{
		/* define the initial opacity of all isosurfaces */
		getKey(3*i+1)->setAlphaL(iniOpacity[i]/255);
	}
...
void TransFunc1DKeys::genKey(float i, const tgt::col4& color)
{
	
	float triangleWidth = (float)1/100;

	keys_.push_back(new TransFuncMappingKey(i-triangleWidth, col4(color[0], color[1],color[2], 0)));
	keys_.push_back(new TransFuncMappingKey(i, color));
	keys_.push_back(new TransFuncMappingKey(i+triangleWidth, col4(color[0], color[1],color[2], 0)));
}

void TransFunc1DKeys::genSquareKey(float i, const tgt::col4& color)
{
	float triangleWidth = (float)1/100;

	keys_.push_back(new TransFuncMappingKey(i-triangleWidth, col4(color[0], color[1],color[2], 0)));
	keys_.push_back(new TransFuncMappingKey(i-triangleWidth, color));
	keys_.push_back(new TransFuncMappingKey(i+triangleWidth, color));
	keys_.push_back(new TransFuncMappingKey(i+triangleWidth, col4(color[0], color[1],color[2], 0)));
}

void TransFunc1DKeys::genSingleKey(float i, const tgt::col4& color)
{
	float triangleWidth = (float)1/30;
	keys_.push_back(new TransFuncMappingKey(i, color));
}
