...
public:
	...
	// Bo:Functions for generating initial 1D TFs
    void setToStandardFunc();
	void initiateOpacity();

	// Bo: generate key as well as neighbor keys to close other isosurfaces
	void genKey(float i, const tgt::col4& color);

	void genSquareKey(float i, const tgt::col4& color);
	void genSingleKey(float i, const tgt::col4& color);
