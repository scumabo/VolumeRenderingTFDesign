...
uniform int examplars[max_feature];
uniform vec3 feature_colors[max_feature];
float vis_scale = 100;    // make sure visbility < 1.0
/***
...
void rayTraversal(in vec3 first, in vec3 last, float entryDepth, float exitDepth) {
    ...
    WHILE(!finished) {
        ...
        if (color.a > 0.0) {
			...
			int classify_iso = 0;
			for(int iso = 0; iso < NumIso; iso++)   // In multiD case NumIso  = # primatives
			{
				if(color.rgb*255 == feature_colors[iso])  // Use color to decide which feature its contribute to
				{
					classify_iso = iso;
					break;
				}
			...
			for(int iso = 0; iso < NumIso; iso++)   // In multiD case NumIso  = # primatives
			{
				if(color.rgb*255 == feature_colors[iso])  // Use color to decide which feature its contribute to
				{
					...
					cumulateOpacity[iso] = cumulateOpacity[iso];
				
				} else {
					// 1. bring geometry similarity
				    cumulateOpacity[iso] = cumulateOpacity[iso] + (texture2D(simlarity_, vec2(classify_iso/NumIso, iso/NumIso)).r/maxValue_)*(1.0f - cumulateOpacity[iso])*definedOpacity;
				}
			...
			result_vis_1_4 = vec4(Visibility[0]/vis_scale, Visibility[1]/vis_scale, Visibility[2]/vis_scale, Visibility[3]/vis_scale);   // if visibility > 1 divide by a scalar
			result_vis_5_8 = vec4(Visibility[4]/vis_scale, Visibility[5]/vis_scale, Visibility[6]/vis_scale, Visibility[7]/vis_scale);
			//result_vis_5_8 = texture2D(simlarity_, vec2(1.0/T_length, 100.0/T_length));
