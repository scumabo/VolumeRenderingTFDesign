...
<p>See CubeProxyGeometry, MeshEntryExitPoints.</p>");
	...
	void autoTFGeneration1D(std::ofstream& out, std::ofstream& shader, std::ofstream& csv);
	void autoTFGeneration2D(std::ofstream& out, std::ofstream& shader, std::ofstream& csv);
	void standardTF(std::ofstream& out, std::ofstream& shader, std::ofstream& csv);


	// Bo: read similarity table
	float* readCSV(std::string fileName, int rows, int cols);
